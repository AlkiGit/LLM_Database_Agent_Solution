...
> **LangGraph** 기반의 에이전트 파이프라인을 통해 자연어 질문을 SQL로 변환하고, 데이터베이스에서 조회한 결과를 분석하여 답변을 제공하는 지능형 AI 시스템입니다.
...
* **🔌 Multi-LLM Support**
  * Google Gemini, Anthropic Claude, OpenAI GPT 등 원하는 모델을 선택하여 사용할 수 있는 유연한 구조입니다. (Dependency Injection 적용)
* **🔗 LangGraph Pipeline**
  * 명확한 상태 관리(State Management)와 에이전트 간의 순차적 제어 흐름을 구현했습니다.
* **🧩 Modular Agent System**
  * Brain, Preprocessing, SQL Generator, Answer 등 각 역할에 특화된 모듈형 에이전트로 구성되어 있습니다.
...
###🤖 에이전트 역할 (Agents)| 에이전트 (Agent) | 역할 (Role) |
| --- | --- |
| **🧠 Brain Agent** | 사용자의 질문을 분석하여 의도를 파악하고 필요한 워크플로우를 결정합니다. |
| **🧹 Preprocessing Agent** | 모호한 자연어 질문을 구체화하거나, 분석에 불필요한 정보를 제거하여 정제합니다. |
| **💻 SQL Generate Agent** | 정제된 질문과 DB 스키마를 바탕으로 실행 가능한 SQL 쿼리를 생성합니다. |
| **📊 Answer Agent** | 생성된 SQL을 실제 DB에서 실행하고, 조회된 데이터를 종합하여 자연어로 답변합니다. |

##🛠️ 설치 및 설정 (Installation)###1. 프로젝트 클론```bash
...
###4. 환경 변수 설정 (.env)프로젝트 루트 경로에 `.env` 파일을 생성하고 아래 내용을 본인의 환경에 맞게 수정하세요.
...
##🚀 사용 방법 (Usage)###1️⃣ LLM 모델 선택`graph_builder.py` 파일 내에서 주석을 해제하여 사용할 LLM 클라이언트를 선택할 수 있습니다.
...
###2️⃣ 실행메인 애플리케이션을 실행하여 에이전트와 대화를 시작합니다.
...
##📂 프로젝트 구조 (Project Structure)```bash
📦 DB_Agent_Multimodel
...
##📦 기술 스택 (Tech Stack)* **Language:** Python 3.9+
...
<p align="center">
<small>README generated with the help of <b>Gemini 3.0 Pro</b></small>
</p>
